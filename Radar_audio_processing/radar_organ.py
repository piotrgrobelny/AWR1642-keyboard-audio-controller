...
def Read_DataAWR(Dataport, configParameters):
    ...
    #Configure audio
    osc = SineLoop(freq=[0, 0, 0, 0], feedback=0.03, mul=.2)
    input_1 = Chorus(osc.mix(1), depth=[1.5, 1.6], feedback=0.5, bal=0.5).out()
    input_2 = Delay(input_1, delay=[.15, .2], feedback=.5, mul=.4).out()
    rev = STRev(input_2, inpos=0.5, revtime=1, cutoff=5000, bal=0.5, roomSize=1, firstRefGain=-3, mul=1, add=0)
    ...
    while True:
        ...
        if magicOK:
            ...
            try:
                tone = int(100 * detObj['y'][0])
                print(tone)
                if "1" in move:
                    high = True
                if "0" in move and high == True:
                    high = False
                    if tone < 8:
                        set_osc_freqs([50, 53, 57, 62])
                    elif tone >= 8 and tone < 16:
                        set_osc_freqs([55, 59, 53, 64])
                    elif tone >= 16 and tone < 24:
                        set_osc_freqs([53, 57, 60, 65])
                    elif tone >= 24 and tone < 32:
                        set_osc_freqs([55, 59, 62, 67])
                    elif tone >= 32 and tone < 40:
                        set_osc_freqs([57, 60, 64, 69])
                    elif tone >= 40 and tone < 48:
                        set_osc_freqs([59, 62, 67, 72])
                    elif tone >= 48 and tone < 56:
                        set_osc_freqs([60, 64, 67, 72])
                    elif tone >= 64 and tone < 72:
                        set_osc_freqs([64, 67, 71, 0])
                    elif tone >= 72 and tone < 80:
                        set_osc_freqs([50, 53, 57, 60])
                    elif tone >= 88 and tone < 96:
                        set_osc_freqs([52, 55, 59, 62])
                    elif tone >= 96:
                        set_osc_freqs([53, 57, 60, 64])
            except:
                pass
